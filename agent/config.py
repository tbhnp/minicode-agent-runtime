...
_DEFAULT_ALLOWED_TOOL_RISKS = (
    "read-only",
    "write",
    "external-side-effect",
)
_VALID_TOOL_RISKS = set(_DEFAULT_ALLOWED_TOOL_RISKS)
...
def _load_allowed_tool_risks(
    agent_tools: dict,
    root_data: dict,
) -> tuple[str, ...]:
    raw = agent_tools.get("allowed_risks", root_data.get("allowed_tool_risks"))
    if raw is None:
        return _DEFAULT_ALLOWED_TOOL_RISKS
    if isinstance(raw, str):
        values = [raw]
    elif isinstance(raw, list):
        values = raw
    else:
        raise ValueError("agent.tools.allowed_risks must be a string list")
    risks: list[str] = []
    for item in values:
        risk = str(item).strip()
        if risk not in _VALID_TOOL_RISKS:
            choices = ", ".join(_DEFAULT_ALLOWED_TOOL_RISKS)
            raise ValueError(
                f"Invalid tool risk {risk!r}; expected one of: {choices}"
            )
        if risk not in risks:
            risks.append(risk)
    return tuple(risks or _DEFAULT_ALLOWED_TOOL_RISKS)
...
def load_config(path: str | Path = "config.toml") -> Config:
    ...
    return Config(
        ...
        reasoner_backend=str(
            agent_cfg.get("reasoner_backend", data.get("reasoner_backend", "default"))
        ).strip().lower()
        or "default",
        ...
        allowed_tool_risks=_load_allowed_tool_risks(agent_tools, data),
        spawn_enabled=bool(
