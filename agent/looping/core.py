...
class AgentLoop:
    ...
    def _assemble_passive_runtime(
        ...
        *,
        ...
        default_reasoner = DefaultReasoner(
            llm=llm_svc,
        ...
        reasoner_backend = (config.llm.reasoner_backend or "default").strip().lower()
        if deps.reasoner is not None:
            self._reasoner = deps.reasoner
        elif reasoner_backend == "default":
            self._reasoner = default_reasoner
        elif reasoner_backend == "langgraph":
            self._reasoner = LangGraphReasoner(default_reasoner)
        else:
            raise ValueError(
                "Unknown reasoner_backend: "
                f"{config.llm.reasoner_backend!r}; expected 'default' or 'langgraph'"
            )
