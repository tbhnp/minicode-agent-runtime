...
from agent.tools.checkpoint import FileCheckpointStore, _default_checkpoint_store
    ...
    async def execute(self, path: str, content: str, **kwargs: Any) -> str:
        try:
            ...
            async def _write() -> str:
                checkpoint = self._checkpoint_store.create(file_path)
                file_path.parent.mkdir(parents=True, exist_ok=True)
                file_path.write_bytes(content.encode("utf-8"))
                return (
                    f"已写入 {len(content)} 字节到 {path}\n"
    ...
    async def execute(
        ...
        try:
            ...
            async def _edit() -> str:
                ...
                checkpoint = self._checkpoint_store.create(file_path)
                file_path.write_bytes(restored_content.encode("utf-8"))
                if diff_text:
                    return (
