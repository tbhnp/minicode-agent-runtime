...
def build_memory_runtime(
    ...
    checkpoint_store = FileCheckpointStore(workspace)
    if not config.memory_v2.enabled:
        register_memory_meta_tools(
            tools,
            write_file_tool=WriteFileTool(checkpoint_store=checkpoint_store),
            edit_file_tool=EditFileTool(checkpoint_store=checkpoint_store),
            restore_checkpoint_tool=RestoreCheckpointTool(
    ...
    register_memory_meta_tools(
        ...
        write_file_tool=WriteFileTool(checkpoint_store=checkpoint_store),
        edit_file_tool=EditFileTool(checkpoint_store=checkpoint_store),
        restore_checkpoint_tool=RestoreCheckpointTool(checkpoint_store=checkpoint_store),
    )
    facade = DefaultMemoryRuntimeFacade(
        port=port,
