...
_DEFAULT_TOOL_RISKS = ("read-only", "write", "external-side-effect")
_VALID_TOOL_RISKS = set(_DEFAULT_TOOL_RISKS)
...
def _as_tool_risks(value: Any, field_name: str) -> tuple[str, ...]:
    if value is None:
        return _DEFAULT_TOOL_RISKS
    if isinstance(value, str):
        ...
        raise ProactiveConfigError(
            f"{field_name} 必须是字符串或字符串数组，当前类型: {type(value).__name__}"
        )
    ...
    for item in values:
        ...
        if risk not in _VALID_TOOL_RISKS:
            raise ProactiveConfigError(
                f"{field_name} 包含非法风险级别: {risk!r}，"
                f"允许值: {', '.join(_DEFAULT_TOOL_RISKS)}"
            )
        if risk not in risks:
            risks.append(risk)
    return tuple(risks or _DEFAULT_TOOL_RISKS)
...
def load_proactive_config(p: dict[str, Any]) -> ProactiveConfig:
    ...
    if "allowed_tool_risks" in agent or "allowed_tool_risks" in at:
        config.allowed_tool_risks = _as_tool_risks(
            _pick(agent, "allowed_tool_risks", at, "allowed_tool_risks"),
            "agent.allowed_tool_risks",
        )
