...
from proactive_v2.tools import TOOL_RISKS, TOOL_SCHEMAS, ToolDeps, dispatch, execute
...
class AgentTick:
    def __init__(
        ...
        *,
        ...
        allowed_tool_risks = getattr(self._cfg, "allowed_tool_risks", None)
        self._tool_executor = ToolExecutor(
            [
                ToolRiskGuardHook(
                    allowed_risks=set(allowed_tool_risks)
                    if allowed_tool_risks
                    else None
                ),
                ShellRmToRestoreHook(),
            ]
        ...
        *,
        ...
        exec_result = await self._tool_executor.execute(
            ToolExecutionRequest(
                ...
                tool_name=tool_name,
                arguments=tool_args,
                tool_risk=TOOL_RISKS.get(tool_name, "read-only"),
                source="proactive",
                session_key=self._session_key,
            ),
