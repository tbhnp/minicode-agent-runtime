...
def test_default_reasoner_denies_tool_when_risk_not_allowed():
    provider = _Provider(
        [
            LLMResponse(content="", tool_calls=[ToolCall("c1", "push", {})]),
            LLMResponse(content="final", tool_calls=[]),
        ]
    )
    tools = ToolRegistry()
    push = _DummyTool("push")
    ...
    assert call["result"] == "tool call denied by risk policy: external-side-effect"
    assert call["pre_hook_trace"][-1]["hook_name"] == "tool_risk_guard"
