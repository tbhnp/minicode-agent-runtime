...
@pytest.mark.asyncio
async def test_message_push_is_denied_when_external_side_effects_are_not_allowed():
    sender = AsyncMock()
    ...
    tick = make_agent_tick(
        cfg=cfg_with(allowed_tool_risks=("read-only", "write")),
        llm_fn=llm,
        sender=sender,
    )
    ...
    assert tick.last_ctx.terminal_action is None
    assert tick.last_ctx.final_message == ""
    assert tick._state_store.tick_step_logs[0]["tool_name"] == "message_push"
    assert (
        tick._state_store.tick_step_logs[0]["tool_result_text"]
        == "tool call denied by risk policy: external-side-effect"
    )
