...
def test_allowed_tool_risks_default():
    assert ProactiveConfig().allowed_tool_risks == (
        "read-only",
        "write",
...
def test_allowed_tool_risks_can_be_overridden():
    cfg = ProactiveConfig(allowed_tool_risks=("read-only", "write"))
    assert cfg.allowed_tool_risks == ("read-only", "write")


def test_load_proactive_config_reads_agent_allowed_tool_risks():
    cfg = load_proactive_config(
        {
            "enabled": True,
            "profile": "daily",
            "agent": {"allowed_tool_risks": ["read-only", "write"]},
        }
    )
    assert cfg.allowed_tool_risks == ("read-only", "write")
