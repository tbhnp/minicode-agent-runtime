...
from agent.tool_hooks import ToolRiskGuardHook
...
def test_tool_risk_guard_denies_unapproved_external_side_effect() -> None:
    executor = ToolExecutor(
        [
            ToolRiskGuardHook(
                allowed_risks={"read-only", "write"},
                message_prefix="blocked by risk policy",
            )
        ]
    )
    ...
    assert result.status == "denied"
    assert result.output == "blocked by risk policy: external-side-effect"
    assert result.pre_hook_trace[-1].hook_name == "tool_risk_guard"
    assert result.pre_hook_trace[-1].decision == "deny"


def test_tool_risk_guard_allows_read_only_tool() -> None:
    executor = ToolExecutor([ToolRiskGuardHook(allowed_risks={"read-only"})])
